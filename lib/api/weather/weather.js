...
async function configureOWMurl(exclude) {
  try {
    ...
    this.logger.trace(`${this._traceStack()} - Configure Open Weather Map url`);

    const url = await Promise.allSettled([
      this._getVaultSecret('OpenWeatherMapKey'),
      this._getVaultSecret('HomeLat'),
      this._getVaultSecret('HomeLong'),
    ]).then((results) => {
      this.logger.trace(
...
async function sunSet(req, res, next) {
  this.logger.debug(`${this._traceStack()} - Sun set api called`);
  ...
  try {
    ...
    if (process.env.MOCK === 'true') {
      ...
      this.logger.trace(`${this._traceStack()} - Calling api`);
    ...
    this.logger.trace(`${this._traceStack()} - Setup the correct sunset time`);
...
async function sunRise(req, res, next) {
  this.logger.debug(`${this._traceStack()} - Sun rise api called`);
  ...
  try {
    ...
    this.logger.trace(`${this._traceStack()} - Setup the correct sunrise time`);
...
async function today(req, res, next) {
  this.logger.debug(`${this._traceStack()} - Current weather API called`);
...
async function willItRain(req, res, next) {
  this.logger.debug(`${this._traceStack()} - Will it rain in 4h API called`);
  ...
  try {
    ...
    try {
      ...
      this.logger.trace(`${this._traceStack()} - No rain data in JSON data`);
    }
...
async function house(req, res, next) {
  ...
  try {
    // Dyson purecool fan
    this.logger.trace(`${this._traceStack()} - Getting latest Dyson data`);
  ...
  try {
    // Netatmo sensors
    this.logger.trace(`${this._traceStack()} - Getting latest Netatmo data`);
  ...
  try {
    ...
    this.logger.trace(`${this._traceStack()} - Construct returning data`);
    let jsonDataObj = [];
