...
async function configureOWMurl(exclude) {
  try {
    ...
    this.logger.debug(`${this.traceStack()} - Configure Open Weather Map url`);

    const url = await Promise.allSettled([
      this.getVaultSecret(process.env.ENVIRONMENT, 'OpenWeatherMapKey'),
      this.getVaultSecret(process.env.ENVIRONMENT, 'HomeLat'),
      this.getVaultSecret(process.env.ENVIRONMENT, 'HomeLong'),
    ]).then((results) => {
      this.logger.debug(
        `${this.traceStack()} - Get sunset from OpenWeatherMap`,
      );
    ...
    this.logger.debug(`${this.traceStack()} - ${err.message}`);
    return err;
  }
}
...
async function sunSet(req, res, next) {
  this.logger.trace(`${this.traceStack()} - Sun set api called`);
  ...
  try {
    ...
    if (process.env.MOCK === 'true') {
      this.logger.debug(
        `${this.traceStack()} - Mock mode enabled, returning mock`,
      );
      apiData = weatherMock;
    } else {
      const url = await configureOWMurl.call(this, 'minutely,hourly,daily');
      if (url instanceof Error) {
        this.logger.error(`${this.traceStack()} - ${url.message}`);
        if (typeof res !== 'undefined' && res !== null) {
          this.sendResponse(res, next, 500, url);
        }
        return url;
      }
      this.logger.debug(`${this.traceStack()} - Calling api`);
      apiData = await helper.callAPIServiceGet(url);
    }
    if (apiData instanceof Error) {
      this.logger.error(`${this.traceStack()} - ${apiData.message}`);
      if (typeof res !== 'undefined' && res !== null) {
        this.sendResponse(res, next, 500, apiData);
      }
      return apiData;
    }

    this.logger.debug(`${this.traceStack()} - Setup the correct sunset time`);
    ...
    if (typeof res !== 'undefined' && res !== null) {
      this.sendResponse(res, next, 200, { sunset: sunSetTime });
    }
    return sunSetTime;
  } catch (err) {
...
async function sunRise(req, res, next) {
  this.logger.trace(`${this.traceStack()} - Sun rise api called`);
  ...
  try {
    ...
    this.logger.debug(`${this.traceStack()} - Setup the correct sunrise time`);
    ...
    if (typeof res !== 'undefined' && res !== null) {
      this.sendResponse(res, next, 200, { sunrise: sunriseTime });
    }
    return sunriseTime;
  } catch (err) {
    this.logger.error(`${this.traceStack()} - ${err.message}`);
...
async function today(req, res, next) {
  this.logger.trace(`${this.traceStack()} - Current weather API called`);
  ...
  try {
    ...
    if (typeof res !== 'undefined' && res !== null) {
      this.sendResponse(res, next, 200, jsonDataObj);
    }
    return jsonDataObj;
  } catch (err) {
    this.logger.error(`${this.traceStack()} - ${err.message}`);
...
async function willItRain(req, res, next) {
  this.logger.trace(`${this.traceStack()} - Will it rain in 4h API called`);
  ...
  try {
    ...
    this.logger.debug(
      `${this.traceStack()} - Filter data for only next 4 hours`,
    );
    ...
    try {
      ...
      this.logger.debug(`${this.traceStack()} - No rain data in JSON data`);
    }
    ...
    if (typeof res !== 'undefined' && res !== null) {
      this.sendResponse(res, next, 500, jsonDataObj);
    }
    return jsonDataObj;
  } catch (err) {
    this.logger.error(`${this.traceStack()} - ${err.message}`);
...
async function house(req, res, next) {
  this.logger.trace(`${this.traceStack()} - Internal house weather API called`);
  ...
  try {
    // Dyson purecool fan
    this.logger.debug(`${this.traceStack()} - Getting latest Dyson data`);
    apiURL = `${process.env.ALFRED_DYSON_SERVICE}/sensors/current`;
    mainBedRoomData = await helper.callAlfredServiceGet.call(this, apiURL);
  ...
  try {
    // Netatmo sensors
    this.logger.debug(`${this.traceStack()} - Getting latest Netatmo data`);
    apiURL = `${process.env.ALFRED_NETATMO_SERVICE}/sensors/current`;
    restOfTheHouseData = await helper.callAlfredServiceGet.call(this, apiURL);
  ...
  try {
    ...
    this.logger.debug(`${this.traceStack()} - Construct returning data`);
    let jsonDataObj = [];
    ...
    if (jsonDataObj.length === 0) {
      this.logger.trace(
        `${this.traceStack()} - No house weather results found`,
      );
    }
    if (typeof res !== 'undefined' && res !== null) {
      this.sendResponse(res, next, 500, jsonDataObj);
